---
title: "国密"
date: 2020-03-04T16:21:44+08:00
draft: false
summary: "国密简介"
...
2. 步骤：
   ...
   b. 使用 SM3 计算杂凑值 $$e=H_v(\overline{M})$$
...
1. `ec_param_new`初始化椭圆曲线，参数p,a,b是确定一条椭圆曲线的参数，n为基点G的阶。p是素数，一般是指有限域中$$F_P$$元素的个数。a,b确定一条椭圆曲线。对于基点$$G=(x_G,y_G) \in E(F_p)，G != 0$$，对于参数n。椭圆曲线的定义为：$$E(F_p) = \{(x,y)|x,y \in F_p, y^2 =x ^3 +ax+b\} \cup \{O\}$$，O 是无穷远点椭圆曲线 $$E(F_p)$$ 上的点的数目称为椭圆曲线 $$E(F_p)$$ 的阶。
2. `ec_param_init`初始化椭圆曲线参数。
3. `sm2_ec_key_new`和`sm2_ec_key_init`新建一个key和key的初始化，根据私钥d和基点G通过`xy_ecpoint_mul_bignum`函数生成私钥。
4. 初始化签名信息
